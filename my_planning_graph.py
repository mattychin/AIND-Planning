...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        a_nodes = []  # create an empty list to contain the action nodes, a_nodes

        for action in self.all_actions:
            a_node = PgNode_a(action)
            # Check to see if preconditions of action are satisfied by looking at
            # whether the prenodes are a subset of the previous S level
            if a_node.prenodes.issubset(self.s_levels[level]):
                # Connect A node to S node instances
                a_nodes.append(a_node)
                for s_node in self.s_levels[level]:
                        s_node.children.add(a_node)
                        a_node.parents.add(s_node)

        self.a_levels.append(a_nodes)  # Construct action level with a_nodes
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            ...
            for s_node in a_node.effnodes:
                s_nodes.add(s_node)
                s_node.parents.add(a_node)      # make the action node a parent
                a_node.children.add(s_node)     # make the effect node a child of the action node
    ...
    def h_levelsum(self) -> int:
        ...
        level_sum = 0  # this will hold the sum of level costs
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    if goal == state.literal:
                        goal_located = True
                        level_sum += level  # add up the level costs
                        break
                if goal_located:
                    break
